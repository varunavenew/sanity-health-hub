...
const FertilitetMagasin = ({ isChatOpen }: PageProps) => {
  ...
  const fertilitySpecialists = useMemo(
    () => specialists.filter((s) => specialistMatchesCategory(s, "fertilitet")).slice(0, 6),
    [specialists]
  );
  ...
  return (
    <PageLayout isChatOpen={isChatOpen}>
      ...
      <section className="bg-brand-warm pt-8 pb-0">
        <div className="flex flex-col lg:flex-row lg:h-[78vh] lg:min-h-[600px]">
          <div className="lg:w-3/5 h-[45vh] lg:h-full relative">
            <AssetImg src={fertilitetImages.hero} alt="Fertilitet" className="w-full h-full object-cover" style={{ objectPosition: "center 25%" }} />
            <div className="absolute top-6 left-6 md:top-10 md:left-10 text-white/90 text-xs uppercase tracking-[0.3em] font-light drop-shadow">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
            {fertilitetAudiences.map((a, i) => (
              <article key={a.title} className="border-t border-foreground/30 pt-6">
                <p className="text-xs uppercase tracking-[0.22em] text-muted-foreground font-light mb-3">0{i + 1}</p>
      ...
      <section className="bg-brand-warm py-20 md:py-28">
        <div className="container mx-auto px-6 md:px-16 max-w-6xl">
          <p className="text-xs uppercase tracking-[0.22em] text-muted-foreground font-light mb-6">Notater i margen</p>
      ...
      <section className="bg-background py-20 md:py-28">
        <div className="container mx-auto px-6 md:px-16 max-w-6xl">
          <div className="grid grid-cols-12 gap-6 md:gap-10 mb-14">
            ...
            <div className="col-span-12 md:col-span-9">
              <p className="text-xs uppercase tracking-[0.22em] text-muted-foreground font-light mb-4">Kapittel 05</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10">
            {fertilitetReviews.map((r) => (
              <blockquote key={r.author} className="border-l-2 border-foreground/30 pl-6">
                ...
                <footer className="text-xs uppercase tracking-[0.18em] text-muted-foreground font-light">— {r.author} · {r.date}</footer>
              </blockquote>
            ))}
          </div>
        </div>
