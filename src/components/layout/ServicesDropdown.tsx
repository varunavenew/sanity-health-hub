...
const hasTreatmentPage = (path: string | undefined): boolean => {
...
const ScrollableMenuColumn = ({
  children,
  className,
  scrollbarClassName = "",
  scrollKey,
  showCustomScrollbar = true,
}: ScrollableMenuColumnProps) => {
  ...
  return (
    <div className={`relative ${scrollbarClassName}`}>
      <div
        ...
        className={`${className} ${
          showCustomScrollbar ? "[scrollbar-width:none] [&::-webkit-scrollbar]:hidden" : ""
        }`}
      >
        {children}
      </div>
          ...
          <div className="relative h-full w-full rounded-2xl md:rounded-full bg-white/[0.15]">
            <div
              ...
              style={{
                ...
                transform: `translateY(${scrollbar.top - 8}px)`,
              }}
            />
          </div>
        </div>
  ...
  return (
    <NavigationMenuItem>
      ...
      >
        ...
        <AnimatePresence>
          {isOpen && serviceCategories.length > 0 && (
            <motion.div
