...
export type ContactEmailPayload = {
  ...
  subject: string;
  message: string;
  /** Preferred callback day, if the visitor chose one. */
  when?: string;
  submittedAt: Date;
  senderName: string;
  website: string;
...
const PLACEHOLDER_PATTERN =
  /\{\{\s*(name|email|phone|clinic|subject|message|date|when|website|senderName)\s*\}\}/gi;
...
function wrapPlainEmailHtml(title: string, bodyHtml: string, lang = "en"): string {
  return `<!DOCTYPE html>
...
function contentFromCmsTemplate(
  ...
  const html = wrapPlainEmailHtml(subject, plainTextToHtml(text), lang);
...
const DEFAULT_CONFIRMATION_SUBJECT = "Takk for at du kontaktet CMedical";

const DEFAULT_CONFIRMATION_BODY = `Hei {{name}},
...
export function buildConfirmationEmailContent(
  ...
  return contentFromCmsTemplate(subjectTemplate, bodyTemplate, values, "nb");
}
