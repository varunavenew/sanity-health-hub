...
async function run() {
    ...
    for (const doc of treatments) {
        ...
        try {
            ...
            // No source process → the fields should not exist on the doc.
            const hasField =
                (doc.flow !== undefined && doc.flow !== null && !(Array.isArray(doc.flow) && doc.flow.length === 0)) ||
                (doc.flowImage !== undefined && doc.flowImage !== null) ||
                (doc.flowImageAlt !== undefined && doc.flowImageAlt !== null);
            ...
            if (DRY_RUN) {
                console.log(`✓ Would unset flow, flowImage, and flowImageAlt: ${label}`);
            } else {
                await sanityClient.patch(doc._id).unset(["flow", "flowImage", "flowImageAlt"]).commit();
                console.log(`✓ Unset flow, flowImage, and flowImageAlt fields: ${label}`);
            }
            unset++;
        } catch (err) {
            errors++;
    ...
    console.log(`Flow Unset:    ${unset}${DRY_RUN ? " (dry run — not committed)" : ""}`);
