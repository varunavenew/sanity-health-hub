...
// Dynamically generate a mocked version of treatmentContent.ts without image/media imports
// to bypass Node ESM loader's ERR_UNKNOWN_FILE_EXTENSION error on non-JS imports.
const srcPath = path.resolve(__dirname, '../../src/data/treatmentContent.ts')
const tmpPath = path.resolve(__dirname, './treatmentContent.tmp.ts')

const originalCode = fs.readFileSync(srcPath, 'utf-8')
const mockedCode = originalCode.replace(
    /import\s+(\w+)\s+from\s+['"]([^'"]+\.(?:jpg|jpeg|png|webp|gif|svg|mp4))['"]/g,
    'const $1 = { src: "" }'
)
fs.writeFileSync(tmpPath, mockedCode, 'utf-8')
...
async function main() {
    let treatmentContent: any
    try {
        const mod = await import('./treatmentContent.tmp')
        treatmentContent = mod.treatmentContent
    } finally {
        try {
    ...
    const existing: Array<{ _id: string; slug?: string; catSlug?: string }> =
        await sanityClient.fetch(
            `*[_type == "treatment"]{
                ...
                "catSlug": coalesce(category->slug[language == "no"][0].value.current, category->slug.current)
            }`,
        )
    ...
    for (const [key, data] of entries) {
        ...
        const categorySlug = categoryId === 'flere-fagomrader' ? 'ovrige' : categoryId
        const bySlugKey = `${categorySlug}/${leaf}`
