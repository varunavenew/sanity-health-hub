 ...
 *
 * Usage:
 *   cd test && SANITY_TOKEN=<token> npx tsx sanity/migrate-clinic-pages-v2.ts --dry-run
 *   cd test && SANITY_TOKEN=<token> npx tsx sanity/migrate-clinic-pages-v2.ts
 *   cd test && SANITY_TOKEN=<token> FORCE=1 npx tsx sanity/migrate-clinic-pages-v2.ts
 *
 * Flags / env:
 *   --dry-run   print the payloads, write nothing
 *   FORCE=1     also overwrite fields that already have a value
 *   SLUG_EN=1   also write `slug_en` (English URL slug) — enable only if your
 *               Studio's i18nSlugFieldFromTitle uses that field name
 */
import { sanityClient } from "./config";

const DRY = process.argv.includes("--dry-run");
const FORCE = process.env.FORCE === "1";
const WRITE_SLUG_EN = process.env.SLUG_EN === "1";
...
const clinics: ClinicSource[] = [
  {
    ...
    hours: {
      no: "Man–tor 08:00–20:00\nFre 08:00–16:00\nLør 10:00–15:00",
      en: "Mon–Thu 08:00–20:00\nFri 08:00–16:00\nSat 10:00–15:00",
    },
    contactDescription: {
    ...
    description: {
      no: "CMedical Majorstuen er hovedklinikken i Oslo, sentralt plassert i Sørkedalsveien 10 A og B. Her tilbys et bredt spekter av spesialisthelsetjenester, fra gynekologi og fertilitet til ortopedi og urologi. Klinikken er moderne innredet med pasientkomfort i fokus.",
      en: "CMedical Majorstuen is our main clinic in Oslo, centrally located at Sørkedalsveien 10 A and B. It offers a broad range of specialist health services, from gynaecology and fertility to orthopaedics and urology. The clinic is modern, with patient comfort in focus.",
    },
    valueProposition: {
      vp1: { no: "Kort ventetid", en: "Short waiting time" },
      highlight: "08:00–20:00",
      socialProof: { no: "Ingen henvisning", en: "No referral needed" },
    },
    detail: {
      parking: {
        no: "Gateparkering tilgjengelig i nærområdet. Nærmeste parkeringshus er Colosseum Park (2 min gange).",
        en: "Street parking is available nearby. The closest car park is Colosseum Park (2 min walk).",
      },
      publicTransport: {
    ...
    seo: {
      ...
      description: {
        no: "Besøk CMedical Oslo Majorstuen i Sørkedalsveien 10. Se åpningstider, tjenester, spesialister og kontaktinformasjon.",
        en: "Visit CMedical Oslo Majorstuen at Sørkedalsveien 10. See opening hours, services, specialists and contact details.",
      },
    },
  },
  {
    ...
    description: {
      ...
      en: "CMedical Bekkestua is centrally located at Bekkestua in Bærum. The clinic offers gynaecology and skin health in modern, calm surroundings.",
    },
    valueProposition: {
      vp1: { no: "Kort ventetid", en: "Short waiting time" },
    ...
    faqs: [
      ...
      },
      {
        ...
        a: {
          no: "Klinikken tilbyr gynekologi, hudhelse og ernæringsfysiolog.",
          en: "The clinic offers gynaecology, skin health and clinical nutrition.",
        },
      },
      {
    ...
    seo: {
      ...
      description: {
        no: "Besøk CMedical Bekkestua i Bærumsveien 205. Gynekologi og hudhelse, åpningstider og kontaktinformasjon.",
        en: "Visit CMedical Bekkestua at Bærumsveien 205. Gynaecology and skin health, opening hours and contact details.",
      },
    },
  },
  {
    ...
    description: {
      ...
      en: "CMedical Moss is located at Lilleengveien 8 and offers a broad range of services, including gynaecology, orthopaedics, gastrointestinal surgery and physiotherapy.",
    },
    valueProposition: {
      vp1: { no: "Kort ventetid", en: "Short waiting time" },
  ...
  },
  {
    ...
    description: {
      no: "CMedical Moelv ligger i Møllergata 18 og er klinikken i Innlandet. Her tilbys gynekologi, ortopedi, urologi, karkirurgi og allmennmedisin.",
      en: "CMedical Moelv is located at Møllergata 18 and is our clinic in Innlandet. It offers gynaecology, orthopaedics, urology, vascular surgery and general medicine.",
    },
    valueProposition: {
      vp1: { no: "Kort ventetid", en: "Short waiting time" },
    ...
    seo: {
      ...
      description: {
        no: "Besøk CMedical Moelv i Møllergata 18. Gynekologi, ortopedi, urologi og allmennmedisin. Åpningstider og kontaktinformasjon.",
        en: "Visit CMedical Moelv at Møllergata 18. Gynaecology, orthopaedics, urology and general medicine. Opening hours and contact details.",
      },
    },
  },
];
...
function buildFields(c: ClinicSource) {
  ...
  if (WRITE_SLUG_EN)
    fields.slug_en = { _type: "slug", current: c.slugEn || c.slug };
...
async function migrate() {
  ...
  const existing: any[] = await sanityClient.fetch(
    `*[_type == "clinicPage" && (_id in $ids || slug.current in $slugs)]`,
  ...
  for (const c of clinics) {
    const doc =
      existing.find((d) => d.slug?.current === c.slug) ||
      existing.find((d) => d._id === `clinicPage-${c.slug}`);
    ...
    for (const [key, value] of Object.entries(all)) {
      if (FORCE || !doc || isEmpty(doc[key])) set[key] = value;
    }
  ...
  if (!WRITE_SLUG_EN)
    console.log("ℹ️  English slug not written. Re-run with SLUG_EN=1 if your Studio uses `slug_en`.");
}
