#!/usr/bin/env npx tsx
/**
 * Restore ortopedi category hero copy to avenewdemo.
 * Writes production (Vercel) and developer.
 *
 *   cd test && npx tsx sanity/patch-ortopedi-hero-copy.ts
 */
...
async function patchDataset(dataset: "developer" | "production") {
  ...
  const beforeDoc = await sanity.fetch(
    `*[_id==$id][0]{
      ...
      "bullets": landingPage.hero.bullets[].title[language=="no"][0].value
    }`,
    { id: DOC_ID },
  );
  console.log(`\n=== ${dataset} before ===`);
