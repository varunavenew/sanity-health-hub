...
async function run() {
    ...
    for (const doc of treatments) {
        ...
        const targetSlug = SLUG_MAP[doc.slug] || doc.slug;
        const lookupCategory = doc.categorySlug === "ovrige" ? "flere-fagomrader" : doc.categorySlug;
        const urlPath = `/behandlinger/${lookupCategory}/${targetSlug}`;
        let amount = byPath.get(urlPath);
        let source: "path" | "category" = "path";
        if (amount === undefined) {
            const fallbackKey = doc.categorySlug === "ovrige" ? targetSlug : doc.categorySlug;
            const mappedFallbackKey = fallbackKey === "ernaeringsfysiolog" ? "ernaering" : fallbackKey;
            amount = byCategory.get(mappedFallbackKey);
            source = "category";
        }
